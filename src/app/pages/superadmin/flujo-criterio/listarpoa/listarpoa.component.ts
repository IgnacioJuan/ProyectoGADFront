...
import { PoaNoAprobadoDTO } from 'src/app/interface/PoaNoAprobadoDTO';
...
export class ListarpoaComponent implements OnInit {
  ...
  public compete = new PoaNoAprobadoDTO();
  competencias: PoaNoAprobadoDTO[] = [];
  

  filterPost = '';
  dataSource = new MatTableDataSource<PoaNoAprobadoDTO>();
  columnasUsuario: string[] = ['id_poa', 'meta_alcanzar', 'fecha_inicio', 'estado','observacion'];
  ...
  listarNoAprobados(): void {
    this.poasservice.getNoAprobados().subscribe(
      (data: PoaNoAprobadoDTO[]) => {
