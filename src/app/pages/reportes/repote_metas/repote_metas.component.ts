...
generarInformeTotal(): void {
  // Crea el contenido del informe PDF
  const content = [];
  ...
  // Agrega el encabezado como una columna
content.push({
  columns: [
    {
      ...
      width: 80, // Ancho de la imagen en puntos (ajusta según tus necesidades)
      alignment: 'left', // Alineación de la imagen
      margin: [0, 10, 0, 10], // Márgenes superior, derecho, inferior e izquierdo de la imagen
    },
    {
      text: `Fecha de generación: ${fechaGeneracion}`,
      alignment: 'right', // Alineación del texto "Fecha de generación"
      margin: [0, 10, 10, 10], // Márgenes superior, derecho, inferior e izquierdo del texto
    },
  ],
});
content.push({ text: `Generado por: ${this.user.persona.primer_nombre} ${this.user.persona.primer_apellido}`, alignment: 'left' });
// Continúa con el resto del contenido del informe.
  // Agrega el título
  content.push({ text: 'Informe de Cumplimiento de Metas del PDOT', style: 'titulo' });

  // Agrega la tabla de datos (obtenidos de la tabla HTML)
  const table = document.querySelector('table'); // Obtén la tabla HTML
  const headerRows = table!.querySelectorAll('tr[mat-header-row] th'); // Encabezados de la tabla
  const dataRows = table!.querySelectorAll('tr[mat-row]'); // Filas de datos

  const tableData = []; // Almacenará los datos de la tabla

  // Obtén los encabezados de la tabla
  const headerRowData: { text: string; style: string; }[] = [];
  headerRows.forEach((headerCell) => {
    headerRowData.push({ text: headerCell.textContent!.trim(), style: 'tabla-encabezado' });
  });
